...
import Hinhthucthanhtoan from './Payments'
import Dieukhoan from './Terms'
import taikhoanApi from '../../../../api/TaikhoanApi';
import { addthanhtoan } from './PaymentSlice';
function Tour(props) {
  const { id } = useParams()
  ...
  const thanhtien = (gia_te) => {
    var gianguoilon = checkKhuyenmai();
    return ((gianguoilon * nguoilon) + (gia_te * treem));
  }
  ...
  const handleOk2 = async () => {
    if (state.dieukhoan === false) {
      ...
      var tourId = id
      let tongtien = thanhtien(tour_ngay[0].giatreem)
      if (stylepayment === 1) {
        await dispatch(addhoadon({ thanhtien: tongtien, tourId, userId, nguoilon, treem, ngaydi: state.date === "" ? formatlaidate(checkngaydi()) : state.date }));
        setState({
          ...state,
          visible2: false,
          visible: false,
        ...
        dispatch(addthanhtoan({ hoadon: { tourId, userId, nguoilon, treem, ngaydi: state.date === "" ? formatlaidate(checkngaydi()) : state.date }, nguoilon, treem, tongtien, "name": tour_ngay[0].name, "giatreem": tour_ngay[0].price_child, "gianguoilon": tour_ngay[0].price_adult }))
        history.push("/stripe");
      }
    }
  };
  ...
  const { name, sdt, diachi, email, nguoilon, treem } = state
  ...
  return (
    <div id="detail-tour">
                    ...
                    <div className="tt-tour">
                      <table className="w-100">
                                    ...
                                    {state.listdate === "" ? "" :
                                      state.listdate.map(ok => (
                                        <Radio style={radioStyle} key={ok.id} value={ok.id}>
                                          <span onClick={() => { adddate(ok.id) }}>{ok.departure}</span><br />
                                        </Radio>
                                      ))}
                                  </Radio.Group>
                                  <hr />
                        ...
                        <tr>
                          ...
                          <td><span>{ok.departure} ngày</span></td>
                        </tr>
                        <tr>
                          <td><span>Nơi khởi hành:</span></td>
                          <td><span>Hồ Chí Minh</span></td>
                        </tr>
                      </table>
                    </div>
                    <Button className="float-right position-absolute btn-dt" onClick={showModal} variant="contained" color="primary">
                      Đặt tour
                  </Button>
                  </div>
                </div>
      ...
      >
        ...
        {tour_ngay.map(ok => (
          <p key={ok.id}>Số tiền cần phải trả: <strong className="text-danger">{thanhtien(ok.price_child).toLocaleString()}</strong></p>
        ))}
      </Modal>
      <Modal
        title="Đặt tour"
